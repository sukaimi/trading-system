...
- **HTTP Basic Auth**: Dashboard and private API behind nginx Basic Auth (`/etc/nginx/.htpasswd`, username: `trader`). Public routes (no auth): `/ping`, `/agents`, `/static/`, `/ws`, `/api/(market|price-history|equity-history|portfolio|events/recent)`
